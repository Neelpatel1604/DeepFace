...
class Solver(object):
    ...
    def test_attack_single(self, image_tensor):
        """Generate transformed images with and without perturbation from a single input image."""
        self.restore_model(self.test_iters)  # Load the trained generator

        image_tensor = image_tensor.to(self.device)

        with torch.no_grad():
            c_org = torch.zeros(1, self.c_dim).to(self.device)  # Create a zero vector for attributes
            c_trg_list = self.create_labels(c_org, self.c_dim, self.dataset, self.selected_attrs)

        pgd_attack = attacks.LinfPGDAttack(model=self.G, device=self.device, feat=None)

        x_fake_list = [image_tensor]  # Store original image
        for idx, c_trg in enumerate(c_trg_list):
            print(f'Processing attribute {idx + 1}/{len(c_trg_list)}')

            with torch.no_grad():
                gen_noattack, _ = self.G(image_tensor, c_trg)  # Generate output without attack

            # Perform attack
            x_adv, perturb = pgd_attack.perturb(image_tensor, gen_noattack, c_trg)
            x_adv = image_tensor + perturb

            with torch.no_grad():
                gen_adv, _ = self.G(x_adv, c_trg)  # Generate adversarial image

            # Save images
            x_fake_list.append(gen_noattack)  # Normal generation
            x_fake_list.append(gen_adv)       # Adversarial generation

        # Save the concatenated image with & without perturbation
        x_concat = torch.cat(x_fake_list, dim=3)
        result_path = os.path.join(self.result_dir, 'generated_vs_perturbed.jpg')
        save_image(self.denorm(x_concat.data.cpu()), result_path, nrow=1, padding=0)

        print(f"Generated vs. perturbed images saved at: {result_path}")
